...
class ACategoryScraperSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in urls:
            yield scrapy.Request(url=self.url, callback=self.parse, meta={"playwright": True})
    ...
    def parse(self, response):
        ...
        for appointment in open_appointments:
            ...
            location = appointment.css(".upperOpomnikDiv > span:nth-child(2)::text").get()
            time = appointment.css(".contentOpomnik.celotnaSirina > div:nth-child(6) > span.bold::text").get()
